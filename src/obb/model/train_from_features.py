...
from obb.model.custom_model import DetectionModelFeatures
...
if __name__ == '__main__':
    train_dataset = Dataset(path='../../../assets/DOTA_sample_data/split', return_features=True)
    train_data_loader = DataLoader(train_dataset, batch_size=1, shuffle=True)

    model = DetectionModelFeatures().to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for P3, P4, P5, obb, object_class in tqdm(train_data_loader):

            P3 = P3.squeeze(dim=0).to(device)
            P4 = P4.squeeze(dim=0).to(device)
            P5 = P5.squeeze(dim=0).to(device)
            obb = obb.squeeze(dim=0)
            object_class = object_class.squeeze(dim=0)
            if len(object_class) == 0:
                continue

            rep_points_init, rep_points_refine, classification = model(P3, P4, P5)  # forward pass
